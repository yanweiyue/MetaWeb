...
Date: 2023-02-15 16:02:35
LastEditors: shaojinxin shaojinxin@citorytech.com
LastEditTime: 2023-02-15 16:02:41
FilePath: /metaweb_backend/apps/dashboard/color.py
Description: 
...
def three_stop_gradient(start_hex, finish_hex, mid_hex = '#FFFFFF', n = 20):
    ...
    """
    first_grad = pd_linear_gradient(start_hex, mid_hex, int(n/2 + 1)).iloc[:-1, :]
    second_grad = pd_linear_gradient(mid_hex, finish_hex, int(n/2 + 1)).iloc[1:, :]
    return first_grad.append(second_grad, ignore_index=True)
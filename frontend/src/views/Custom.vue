...
const case_detail_item = computed(() => {
  return [
    ...
    {
      name: "Stage",
      val: case_detail.value.stage,
      icon: "el-icon-ali-address"
    },
    {
      name: "Tag",
      val: case_detail.value.tag,
      icon: "el-icon-ali-address"
    },
    {
      name: "Description",
...
//获取数据
const showCase = () => {
  axios(api.query_case({type: "custom"})).then(
      (response) => {
        if (response.status == 200) {
          ...
          drawGlobeThree(app)
          document.getElementById('globeThree').addEventListener('click', (event) => {
            ...
            let stage = case_data.value[instance.instanceId].stage;
            showDetail.value = true;

            axios(api.query_detail({type: "custom", caseid: caseid, userid: userinfo.value.id, stage: stage})).then(
                (response) => {
                  if (response.status == 200) {
                    case_detail.value = response.data;
                    case_detail.value.id = caseid;
                    case_detail.value.stage = stage;
        ...
        <el-row class="full-width" id="detail-description">
          ...
          >
            ...
            <el-descriptions-item v-for="item in case_detail_item" class="detail-item">
              ...
              <div v-if="item.name === 'Tag'">
                <el-tag v-for="tag in item.val">
                  {{tag}}
                </el-tag>
              </div>
              <div v-else-if="item.name === 'Stage'">
                <el-tag>
                  {{item.val === 'mid' ? '中期' : '终期'}}
                </el-tag>
              </div>

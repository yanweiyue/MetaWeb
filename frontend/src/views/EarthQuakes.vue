 ...
 * @LastEditTime: 2023-03-10 18:28:42
 * @FilePath: /metaweb_front/src/views/EarthQuakes.vue
 * @Description: 
 * 
...
// 时间轴相关
config.timeline.startDate = new Date('1000-01-01'); //开始时间（在地內）
config.timeline.surfaceDate = new Date('2000-01-01');//地表象征时间
...
const list_dashboard_content_data = () => {
    axios(api.dashboard_list_content({ fromtype: 3, start_hex: '#007AFF', finish_hex: '#ff0000', mid_hex: '#00FF64', color_length: 30 })).then(
        (response) => {
            if (response.status == 200) {
                ...
                for (let p of dashboard_content_data.value) {
                    ...
                    p.scale = (parseFloat(p.magnitude) - MAG_MIN) / (9.2 - MAG_MIN)
                }
                ...
                window.addEventListener('click', (event) => {
                    if (!dialogVisible.value) {
                        dashboard_content_detail_data.value = dashboard_content_data.value[onClick(event)]
        ...
        <el-dialog v-model="dialogVisible" v-if="dashboard_content_detail_data != null">
            <div class="dialog_body">
                <div class="dialog_left">
                    <h2 style="color:#5FD87E">{{ dashboard_content_detail_data['location'] }}</h2>
                    <p>时间:<el-tag>{{ dashboard_content_detail_data['date'].Format("yyyy-MM-dd") }}</el-tag></p>
                    <p>震级:<el-tag>{{ dashboard_content_detail_data['magnitude'] }}</el-tag> 深度:<el-tag>{{
                        dashboard_content_detail_data['depth_km'] }}KM</el-tag></p>
                    <p>地点:<el-tag>{{ dashboard_content_detail_data['location'] }}</el-tag></p>
                    <p>经度:<el-tag>{{ dashboard_content_detail_data['longitude'] }}</el-tag> 纬度:<el-tag>{{
                        dashboard_content_detail_data['latitude'] }}</el-tag></p>
                </div>
                <!-- <div class="dialog_right"> 
                        <el-image style="width: 256px; height: 256px"
                            :src="dashboard_content_detail_data['album']"

 ...
 * @LastEditTime: 2023-03-10 18:28:56
 * @FilePath: /metaweb_front/src/views/PlanetTech.vue
 * @Description: 
 * 
...
// 时间轴相关
config.timeline.startDate = new Date('1949-01-01');
config.timeline.surfaceDate = new Date('1979-01-01');
...
const list_dashboard_content_data = () => {
    axios(api.dashboard_list_content({ fromtype: 2 })).then(
        (response) => {
...
const flyToNext = () => {
    ...
    let p = dashboard_content_data.value[indexCurrent]
    flyToWithMid(threecamera.value, { lon: p.longitude, lat: p.latitude, alt: p.altitude + 4, id: p.name }, get_dashboard_content_detail)
    indexCurrent = indexCurrent < dashboard_content_data.value.length - 1 ? indexCurrent + 1 : 0
}
    ...
    <div id="vue">
        ...
        <el-dialog v-model="dialogVisible" v-if="dashboard_content_detail_data != null">
            <div class="dialog_body">
                ...
                <div class="dialog_right"> <el-image style="width: 256px; height: 256px"
                        :src="dashboard_content_detail_data['album'][0].url + '/thumb512'"
                        :preview-src-list="[dashboard_content_detail_data['album'][0].url + '/thumb1080']"
                        :zoom-rate="1.2" :initial-index="1" fit="cover" /></div>
            </div>
            <div class="control">
